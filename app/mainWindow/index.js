...
import ZentCashConfig from "./wallet/zentcashConfig";
...
export const directories = [
    `${homedir}/.zentcashwallet`,
    `${homedir}/.zentcashwallet/logs`
];
...
remote.app.setAppUserModelId("wallet.cash.zent");

log.debug(`Zent Cash Wallet started...`);
...
eventEmitter.on("updateRequired", updateFile => {
    ...
    const message = (
        <div>
            ...
            <br />
            <p className={`subtitle ${textColor}`}>
                There&apos;s a new version of Zent Cash Wallet available. Would
...
ipcRenderer.on("fromMain", (event: Electron.IpcRendererEvent, message: any) => {
    ...
    switch (messageType) {
        case "config":
            ...
            configManager = new ZentCashConfig(data.config, data.configPath);
